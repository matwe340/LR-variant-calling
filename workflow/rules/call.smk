...
rule call:
    ...
    shell:
        #"bcftools mpileup --threads {threads} -q 20 -Q 20 -Ou -s {wildcards.individual} --ignore-RG -f {input[2]} {input[0]} -a \"AD,ADF,ADR,DP,SP\" 2> {log[0]} | bcftools call --threads {threads} -a \"GQ\" --ploidy {config[ploidy]} --skip-variants indels -m -Oz -o {output} > {log[1]} 2>&1"
        """
        bcftools mpileup --threads {threads} \
        -q 20 -Q 20 -Ou -s {wildcards.individual} \
        --ignore-RG -f {input[2]} {input[0]} -a \"AD,ADF,ADR,DP,SP\" 2> {log[0]} | \
        bcftools call --threads {threads} -a \"GQ\" --ploidy {config[ploidy]} \
        --skip-variants indels -m -Oz -o {output} > {log[1]} 2>&1
        """
